...
_WeeChat ≥ 2.4, ажурирано у 4.3.0, 4.11.0._
...
Ако је стринг који треба да се декодира неисправан (неисправни char,
одсечени стринг или неисправна испуна), у _*to_ се смешта празан стринг и враћа се -1.

Прихватају се недостајући карактери испуне (`=`) на крају стринга.
...
Израчунава израз и враћа вредност као стринг. Специјалне променљиве у формату `+${xxx}+` се развијају (погледајте табелу испод).

[NOTE]
Почевши од верзије 1.0, подржава се угњеждавање променљивих, на пример: `+${color:${xxx}}+`.
...
| `+${xxx}+` |
| Променљива `xxx` из хеш табеле _extra_vars_.
| >> `+${xxx}+` +
  == `+вредност+`
...
| `+${sec.data.xxx}+` |
| Вредност обезбеђених података `xxx`.
...
| `+${xxx}+` |
| Вредност локалне променљиве `xxx` у баферу.
| >> `+${nick}+` +
  == `+FlashCode+`

| `+${xxx}+` |
| Променљива `xxx` из хеш табеле _pointers_.
| >> `+${buffer}+` +
  == `+0x1234abcd+`
...
У случају грешке, динамички стринг остаје неизмењен.
...
* _type_: тип опције; иза било ког типа може да се стави `+|themable+` чиме се
  опција обележава као она која може да се поставља темом, тако да се може поставити
  командом `/theme` (опције типа _color_ увек могу да се поставе темом) _(WeeChat ≥ 4.10.0)_:
...
Функције које додају преиначавања боје (и осталих опција које могу да
се поставе темом) уграђеним темама. Погледајте одељак
link:weechat_user.sr.html#themes[корисничко упутство / Теме] за употребу
на страни крајњег корисника и команду `+/theme+` command.

==== theme_register

_WeeChat ≥ 4.10.0._

Региструје допринос преиначавања опције у оквиру теме. Додатак позиваоца
је власник доприноса; накнадни позиви од истог додатка и под истим именом
теме се спајају у постојећи допринос (у случају дупликата, каснији кључеви
побеђују).

Када се позивајући додатак уклања из меморије, сви његови доприноси се
аутоматски уклањају из сваке теме.
...
* _name_: име теме (на пример `+light+` или произвољно име)
* _overrides_: хештабела која мапира комплетна имена опције
  (нпр. `+irc.color.input_nick+`) у њихове стринг вредности; позивалац
  задржава власништво и може да ослободи хештабелу одмах након позива

Повратна вредност:

* показивач на регистровану тему (постојећу или новокреирану), NULL у
  случају грешке
...
[NOTE]
Команда `/theme apply` ће моћи да постави само опције којима је
постављена заставица _themable_. Све `*.color.*` опције могу
 подразумевано да се поставе темом; стринг опције које држе
`+${color:...}+` референце се експлицитно укључују. Ставке које
циљају на опције које не могу да се поставе у теми се током
примењивања прескачу у тишини, при чему се у_core_ бафер уписује
упозорење о томе.

[NOTE]
Када се позива из скрипте, допринос се аутоматски брише у тренутку
када се скрипта уклони из меморије (није потребно ручно чишћење).
...
| id
| Идентификатор линије: датум и време штампања са прецизношћу до микросекунде.
| Број линије ("y").
| `+1786886000000000+`
...
| Показивач: бафер.
| Приказује префикс за сваку линију која је промењена.
...
| _chat_line_id | Идентификатор линије ^(4)^ _(WeeChat ≥ 4.11.0)_.
| "1786886000000000" | "-1"
...
| _chat_line_date_printed | Датум/време линије ^(5)^.
| "1313237175" | "0"

| _chat_line_date_usec_printed | Микросекунде датума/времемна приказа линије ^(5)^.
| "123456" | "0"
...
^(4)^ На баферима са форматираним садржајем, ово је датум када је WeeChat
додао линију у бафер, са прецизношћу до микросекунде (веће или једнако од
"_chat_line_date"); на баферима са слободним садржајем, ово је број линије
("_chat_line_y"). +
^(5)^ Застарело од верзије WeeChat 4.11.0 (још увек је ту из разлога компатибилности):
изводи се из идентификатора линије ("_chat_line_id") и има вредност "0" на баферима са
слободним садржајем.
...
** _prefix_for_each_line_: 1 ако се префикс приказује за сваку линију у баферу (подразумевано), иначе 0 _(WeeChat ≥ 4.11.0)_
...
** _input_get_any_user_data_: 1 ако се било који кориснички подаци, укључујући команде, шаљу функцији повратног позива уноса, иначе 0
...
Повратна вредност „long long” особине бафера.
...
* _buffer_: показивач на бафер
* _property_: име особине:
** _id_: јединствени идентификатор
** _lines_last_id_assigned_: последњи јединствени идентификатор употребљен у овом баферу за линију _(WeeChat ≥ 4.11.0)_
** _nicklist_last_id_assigned_: последњи јединствени идентификатор употребљен у овом баферу за групу или надимак

Повратна вредност:

* „long long” вредност особине
...
| prefix_for_each_line | 4.11.0 | "0" или "1"
| "0" да се префикс сакрије у свим линијама бафера, "1" да се префикс види у свим линијама
  (подразумевано за нови бафер).
...
* _buffer_: показивач на бафер
* _id_: идентификатор линије: на баферу са форматираним садржајем, ово је
  датум и време штампања са прецизношћу до микросекунде (број микросекунди од
  почетка Епохе); на баферу са слободни садржајем, ово је број линије ("y")
...
Додаје „long” променљиву у ставку инфолисте.
...
Додаје „long long” вредност у ставку инфолисте.
...
Враћа вредност „long” променљиве у текућој ставки инфолисте.
...
* _infolist_: показивач на инфо листу
* _var_: име променљиве (мора бити типа „long”)

Повратна вредност:

* „long” вредност променљиве
...
Враћа вредност „long long” променљиве у текућој ставки инфолисте.
...
* _infolist_: показивач на инфо листу
* _var_: име променљиве (мора бити типа „long long”)
