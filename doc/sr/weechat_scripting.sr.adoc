...
у link:weechat_plugin_api.sr.html[API референтном приручнику WeeChat додатака ^↗^^].
...
API скриптовања је скоро у потпуности исти као и API C додатака. Можете да погледате link:weechat_plugin_api.sr.html[WeeChat референца API додатака ^↗^^] у вези детаља сваке функције у API: прототип, аргументи, повратне вредности, примери.
...
# За остале језике, измените „python” именом свој језика (perl/ruby/lua/tcl/guile/javascript/php)
----
...
weechat.prnt("", "WeeChat конфиг дир: %s" % weechat.info_get("weechat_config_dir", ""))
