...
| Logger додатак: компресија ротирајућих лог фајлова (gzip). +
  Релеј додатак: компресија порука (WeeChat -> клијент) са https://zlib.net/[zlib ^↗^^]
  (api и weechat протоколи). +
  Script додатак: читање индекс фајла репозиторијума (gzip).
|===

[NOTE]
^(1)^ Име долази из Debian GNU/Linux Trixie дистрибуције, верзија и
...
| Logger додатак: компресија ротирајућих лог фајлова (zstandard). +
  Релеј додатак: компресија порука (WeeChat -> клијент) са https://facebook.github.io/zstd/[Zstandard ^↗^^]
  (api и weechat протоколи).
...
| Ставка                | Пример                 | Опис
| time                  | `12:55`                | Време.
| buffer_last_number    | `5`                    | Број најновијег бафера (може да се разликује од `buffer_count` ако је опција <<option_weechat.look.buffer_auto_renumber,weechat.look.buffer_auto_renumber>> `off`).
...
|===
| Ставка                       | Пример                   | Опис
| buffer_count                 | `5`                      | Укупан број отворених бафера.
...
| log0 … log9      | Ниво лога за линију (погледајте команду <<command_logger_logger,/logger>>).
...
include::includes/key_bindings.sr.adoc[]
...
Тема је именовани пакет преиначавања опција који може да се примени
командом <<command_weechat_theme,/theme>>. WeeChat се испоручује са
`„светлом”` темом која је прилагођена терминалима са светлом позадином
и подржава пролазно учитавање кориснички дефинисаних фајлова теме.

[[themes_themable_options]]
==== Опције које могу да се поставе темом

Теме могу да поставе само опције које су означене као _themable_. Све `*.color.*`
опције подразумевано могу да се поставе темама; неколико стринг опција које
чувају изразе формата са `+${color:...}+` референцама (као што су
`+weechat.color.chat_nick_colors+`, `+weechat.look.prefix_error+` или
`+buflist.format.*+` формати) мора експлицитно да се укључе.

Комплетну површину области која може да се подешава темом можете да видите из
<<command_fset_fset,/fset>> бафера употребом `+t:themable+` филтера.

[[themes_apply]]
==== Примењивање теме

Ако желите да пређете на уграђену тему светле позадине:

----
/theme apply light
----

Најпре се текуће стање сваке опције које може да се поставља темом чува
у резервни фајл теме под именом `+backup-ГГГГММДД-ЧЧММСС-uuuuuu+` у
директоријум `+themes+` унутар WeeChat конфигурационог директоријума,
тако да се претходни изглед може вратити у било које време:

----
/theme apply backup-ГГГГММДД-ЧЧММСС-uuuuuu
----

Креирање резервне копије може да се искључи (не препоручује се):

----
/set weechat.look.theme_backup off
----

Ако је прављење резервне копије укључено, а фајл резервне копије не
може да се упише, примењивање се прекида пре измене било које опције.

Име последње примењене теме се смешта у`+weechat.look.theme+`
(чисто информације ради; не примењује се поново приликом покретања).

[[themes_reset]]
==== Ресетовање на подразумевано

Ако желите да вратите изгледа на онај који се испоручује уз WeeChat,
ресетујте сваку опцију која може да се постави темом на њену
...
Најпре се записује резервна копија (иста капија као за `+/theme apply+`);
ако прављење резервне копије не успе, ресетовање се прекида пре измене
било које опције. `+weechat.look.theme+` се такође брише.

[[themes_save_delete]]
==== Чување и брисање корисничких тема

Чување тренутних вредности опција које могу да се мењају темом као нов фајл теме:

----
/theme save мојатема
----

Уписује се свака опција која може да се постави темом, тако да се у
фајлу налази све што је потребно и примењује потпуно исти изглед на
било којем WeeChat програму, без обзира на његову текућу конфигурацију.

Резервисана имена (имена уграђених тема као што су `+light+` и било која
имена која почињу са `+backup-+`) се не прихватају. Фајлови се налазе у
`+${weechat_config_dir}/themes/<име>.theme+`.

Преименовање корисничке теме (типична употреба: чување корисне аутоматске
резервне копије под смисленим именом):

----
/theme rename backup-20260525-094210-123456 мојбекап
----

Уграђене теме се не држе у фајлу и не могу да се преименују; циљно име не сме
се поклопи са уграђеним именом или да почиње са `+backup-+`, и циљни фајл
не сме већ да постоји. Поље `+[info]+` `+name+` унутар фајла се поново
исписује тако да команда `/theme info` конзистентно пријављује ново име.

Брисање корисничке теме:

----
/theme del мојатема
----

Ово уклања фајл са диска; уграђене теме не могу да се обришу.

[[themes_file_format]]
==== Формат фајла теме

Фајл теме је личи на INI фајл са два одељка:
...
`+[info]+` су информациони метаподаци који се приказују командом `/theme info`.
`+[options]+` садржи стварна преиначавања; кључеви су пуна имена опција
који би се појавили у `/set`. Стринг вредности могу да се поставе у знаке
навода или полунавода; у време парсирања, знакови навода се уклањају. Опције
које не могу да се поставе темом наведене у фајлу теме се одбијају у
време примењивања теме и упозорење се логује у основни бафер (тако да
`.theme` фајл који је увезен из неког извара којем се не верује не може
да препише лозинке, листе аутоматског учитавања или команде за покретање).

Фајлови корисничких тема се никада не кеширају: приликом сваког извршавања
`/theme apply <име>` фајл се парсира, примењује и ослобађа.

[[themes_resolution]]
==== Редослед разрешавања

Када се изврши `+/theme apply <име>+`:

* Ако постоји `+${weechat_config_dir}/themes/<име>.theme+`, фајл се
  парсира и користи (он заклања било коју уграђену тему са истим именом).
* У супротном, консултује се регистар уграђених тема; језгро, додаци и
  скрипте програмски региструју уграђене теме (погледате документацију
  додатак и скриптовања за `+weechat_theme_register+`).

Ако ниједан извор не обезбеђује име, примењивање се одбија.
...
Ако желите да се лог фајлови креирају по датуму, у маски можете да употребите спецификаторе датума/времена (за формате, погледајте `man strftime`, extra specifiers are supported, see link:weechat_plugin_api.sr.html#_util_strftimeval[WeeChat plugin API reference / util_strftimeval ^↗^^]), на пример:
...
За комплетну листу подржаних опција, погледајте команду <<command_weechat_secure,/secure>>.
...
За помоћ у вези са форматом, погледајте опцију <<option_irc.server_default.autojoin,irc.server_default.autojoin>>.
...
почињу истим именом (погледајте команду <<command_weechat_filter,/filter>>):
...
* _api_: HTTP REST API, користе га WeeChat и удаљени интерфејси да прикажу и да имају интеракцију
  са WeeChat
* _weechat_: протокол који удаљени интерфејси користе да прикажу и да имају интеракцију са
  WeeChat.

За _api_ и _weechat_ протоколе, погледајте листу удаљених интерфејса на
https://weechat.org/about/interfaces/[овој страници ^↗^^].
...
Уз лозинку, као други фактор за _api_ и _weechat_ протоколе може да се користи TOTP
(Time-based One-Time Password) (једнократна лозинка базирана на времену).
...
Релеј додатак може да шаље податке удаљеном WeeChat програму или интерфејсу користећи
HTTP REST API протокол.

API можете да погледате и тестирате на мрежи:
https://weechat.org/api/[WeeChat Релеј API ^↗^^].

Можете да се повежете са програмом WeeChat или са удаљеним интерфејсом, погледајте
...
URI мора да се завршава са „/weechat” за _irc_ и _weechat_ протоколе, а са „/api”
за _api_ протокол.
...
| id                 | стринг    | Идентификатор линије: датум и време штампања са прецизношћу до милисекунде (број линије за бафер са слободним садржајем).
...
....
┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│1.weechat│322/322 скрипти (филтер: {x}) | Сортирање: i,p,n | Alt+тастер/унос: i=инсталација, r=уклањање, l=учи>>│
