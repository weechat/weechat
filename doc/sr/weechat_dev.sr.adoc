...
|          script/   | Unit тестови за script додатак.
...
|    core-signal.c              | Сигнал функције.
|    core-string.c              | Функције над стринговима.
|    core-sys.c                 | Системске функције.
|    core-theme.c               | Теме (пакети преиначавања опција).
|    core-theme-builtin.c       | Уграђене теме: основне опције.
...
|    gui-bar-item-custom.c      | Ставке прилагођене траке.
|    gui-bar-window.c           | Прозори трака.
|    gui-buffer.c               | Бафери.
|    gui-chat.c                 | Функције разговора (приказ поруке, ...).
...
|       buflist-theme.c              | Buflist допринос за уграђене теме.
|    charset/                        | Charset додатак.
...
|       exec-theme.c                 | Exec допринос за уграђене теме.
|    fifo/                           | Fifo додатак.
...
|       fset-theme.c                 | Fset допринос за уграђене теме.
...
|       irc-theme.c                  | IRC допринос за уграђене теме.
...
|       logger-theme.c               | Logger допринос за уграђене теме.
|    lua/                            | Lua додатак.
...
|       relay-theme.c                | Релеј допринос за уграђене теме.
...
|       script-theme.c               | Допринос управљача скриптама за уграђене теме.
...
|       spell-theme.c                | Допринос провере правописа за уграђене теме.
|    tcl/                            | Tcl додатак.
...
|       trigger-theme.c              | Допринос окидача за уграђене теме.
|    typing/                         | Typing додатак.
...
|       xfer-theme.c                 | Xfer допринос за уграђене теме.
...
|          calc-fuzzer.cc                         | Fuzz тестирање: калкулација израза.
|          crypto-fuzzer.cc                       | Fuzz тестирање: криптографске функције.
|          eval-fuzzer.cc                         | Fuzz тестирање: израчунавање израза.
|          secure-fuzzer.cc                       | Fuzz тестирање: обезбеђени подаци.
|          string-fuzzer.cc                       | Fuzz тестирање: стрингови.
|          utf8-fuzzer.cc                         | Fuzz тестирање: UTF-8.
|          util-fuzzer.cc                         | Fuzz тестирање: услужне функције.
...
|          test-core-theme.cpp                    | Тестови: теме.
...
|             test-gui-curses-term.cpp            | Тестови: функције терминала (Curses интерфејс).
...
// TRANSLATION MISSING
|             test-irc-completion.cpp             | Tests: IRC completions.
...
|             test-relay-bar-item.cpp             | Тестови: ставке траке за Релеј додатак.
|             test-relay-client.cpp               | Тестови: клијенти релеја.
...
|          script/                                | Корен unit тестова за script додатак.
|             test-script-repo.cpp                | Тестови: script репозиторијум.
...
|             test-xfer.cpp                       | Тестови: xfer функције.
|             test-xfer-chat.cpp                  | Тестови: DCC чет.
...
|          key_bindings.XX.adoc                 | Пречице на тастатури (фајл је укључен у корисничко упутство и пушкице).
...
Ауторска права и лиценца морају да се поставе употребом SPDX (System Package Data Exchange),
следећи најбољу праксу у REUSE Software
(погледајте https://reuse.software/spec/[REUSE спецификацију ^↗^^]).

Сви фајлови у репозиторијуу морају да имају исправне информације о ауторским правима
и лиценцирању. +
Када информације не могу да се поставе у сам фајл, можете да употребите REUSE.toml фајл
у корену стабла пројекта.
...
** Коментар мода почне великим словом, изузетак су идентификатори.
** Комплетна реченица мора да се заврши тачком.
** Коментари који описују поља у struct могу да буду исписани само малим словима.
...
foo ()
{
    ...
    /*
     * This is a multi-line comment: this is a very long description about
     * next block of code.
     */
    i = 2;
    printf ("%d\n", i);
