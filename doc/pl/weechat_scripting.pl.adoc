...
* _name_: string, wewnętrzna nazwa skryptu
* _author_: string, autor skryptu
* _version_: string, wersja
* _license_: string, licencja
* _description_: string, krótki opis skryptu
* _shutdown_function_: string, nazwa funkcji wywoływanej podczas wyładowania skryptu
  (może być pusty ciąg)
* _charset_: string, kodowane skryptu (jeśli skrypt jest napisany
...
# Dla innych języków, zmień "python" na swój język (perl/ruby/lua/tcl/guile/javascript/php)
----
