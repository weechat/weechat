...
[NOTE]
^(1)^ Nazwa pochodzi z repozytorium dystrybucji Debian GNU/Linux Trixie distribution,
...
include::includes/key_bindings.pl.adoc[]
...
Motyw to nazwany pakiet ustawień, które można zastosować za pomocą 
polecenia <<command_weechat_theme,/theme>>. WeeChat zawiera wbudowany
„jasny” motyw dostosowany do terminali z jasnym tłem i obsługuje
motywy zdefiniowane przez użytkownika, ładowan z plików.

[[themes_themable_options]]
==== Ustawienia motywów

Motywy mogą tylko ustawiać opcje oznaczone jako _themable_. Wszystkie opcje
`*.color.*` domyślnie mogą być ustawiane przez motywy; kilka opcji, których 
wartości są ciągami, mogących przechować wyrażenia formatujące z odwołaniami 
`+${color:...}+` (jak `+weechat.color.chat_nick_colors+`, `+weechat.look.prefix_error+`
lub format `+buflist.format.*+`) są również uwzględnione.

Możesz wyświetlić wszystkie opcjie, jakie mogą być ustawione przez motyw
w buforze <<command_fset_fset,/fset>> używając filtru `+t:themable+`.

[[themes_apply]]
==== Wybieranie motywu

W celu wybrania wbudowanego motywu o jasnym tle:

----
/theme apply light
----

Aktualny stan wszystkich opcji ustawanych przez motyw zostaną wcześniej
zapisane do pliku z kopią zapasową motywu o nazwie jak `+backup-YYYYMMDD-HHMMSS-uuuuuu+`
w katalogu `+themes+` wewnątrz katalogu z konfiguracją WeeChat, dzięki
czemu wcześniejszy wygląd można zawsze przywrócić za pomocą:

----
/theme apply backup-YYYYMMDD-HHMMSS-uuuuuu
----

Tworzenie kopi zapasowej można wyłąćzyć (nie zalecane):

----
/set weechat.look.theme_backup off
----

Jeśli kopie zapasowe są włączone i nie mogą zostać zapisane, wczytywanie
motywu jest przerywane zanim jakakolwiek opcja zostanie zmieniona.

Nazwa ostatnio użytego motywu jest przechowywana w `+weechat.look.theme+` 
(wyłącznie informacyjnie; wartość nie ma wpływu na motyw ladowany podczas 
uruchamiania).

[[themes_reset]]
==== Przywracanie wartości domyślnych

W celu przywrócenia wyglądu dostaczanego z WeeChat należy przywrócić
wartości domyślne opcji motywów:

----
/theme reset
----

Najpierw jest robione kopia zapasowa (tak samo jak przy `+/theme apply+`);
w przypadku niepowodzenia operacja jest przerywana przed zmianą
jakiejkolwiek wartości.
Wartość `+weechat.look.theme+` jest również czyszczona.

[[themes_save_delete]]
==== Zapisywanie i kasowanie motywów użytkowników

Zapisanie obecnego wyglądu jako nowy motyw użytkownika:

----
/theme save mytheme
----

Każda opca motywu jest zapisywana, dzięki czemu każdy motyw jest
kompletny i będzie wyglądać tak samo na każdym WeeChat, bez
względu na obecną konfigurację.

Nazwy zarezerwowane (wbudowane motywy jak `+light+` i każdna
nazwa zaczynająca się od `+backup-+`) zostaną odrzucone. Pliki
są przechowywane w `+${weechat_config_dir}/themes/<name>.theme+`.

Zmiana nazwy motywu użytkownika (zwykłe użycie: trzymaj przydatne
kopie zapasowe z nazwami mającymi sens):

----
/theme rename backup-20260525-094210-123456 mójmotyw
----

Wbudowane motywy nie posiadają plików i nie można zmienić ich nazwy;
nowa nazwa nie może być nazwą wbudowanego motywy ani zaczynać się
od `+backup-+`, oraz plik z taką nazwą nie może już istnieć. Pole
`+[info]+` `+nazwa+` wewnątrz pliku zostanie nadpisane dzięki czemu
`/theme info` zwróci poprawną wartość.

Usuwanie motywu użytkownika:

----
/theme del mytheme
----

Usuwa to plik na dysku; wbudowane motywy nie mogą zostać usunięte.

[[themes_file_format]]
==== Format pliku motywu

Plik z motywem ma format pliku podobny INI z dwoma sekcjami: 
...
`+[info]+` to metadane pokazywane przez `/theme info`. `+[options]+`
przechowuje aktualne wartości; klucze to pełne nazwy opcji, które
pojawiły by się w `/set`. Wartości ciągów mogą być ujęte w podwójnie lub
pojedyncze cudzysłowy; cudzysłowy są pomijane podczas parsowania. Opcje
nie wapierane przez motywy są pomijane podczas parsowania i wyświetlane
w głównym buforze (dzięki czemu plik `.theme` zaimportowany z niezaufanego
Źródła nie może nadpisać haseł, list automatycznego ładowania ani komend
wykonywanych przy uruchomieniu).

Motywy użytkowników nie są przechowywane: przy każdym wykonaniu
`/theme apply <nazwa>` plik jest parsowany, zmiany są stosowane i plik 
jest zwalniany.

[[themes_resolution]]
==== Kolejność wykonywania

Kiedy polecenie `+/theme apply <nazwa>+` jest wykonywane:

* Jeśli plik `+${weechat_config_dir}/themes/<name>.theme+` istnieje,
  jest on parsowany i używany (przysłania każdą wbudowany motyw o tej
  samej nazwie).
* W innym przypadku sprawdzany jest rejestr wbudowanych motywów; wbudowane
  motywy są automatycznie rejestrowane przez WeeChat, wtyczki i skrypty
  (zobacz dokumentacje wtyczek i skryptów na temat `+weechat_theme_register+`).

Jeśli żadne ze źródeł nie dostarcza nazwy, motyw nie jest używany.
...
extra specifiers are supported, see link:weechat_plugin_api.en.html#_util_strftimeval[WeeChat plugin API reference / util_strftimeval ^↗^^]),
na przykład:
...
| Pasek z typem `root` jest wyświetlany dokładnie jeden raz na ekranie, poza
  ...
  podzielisz jednorazowo ekran (używając `/window splith` lub `/window splitv`)
...
| id                 | ciąg     | Identyfikator linii: data i czas wyświetlenia z dokładnością do milisekund (numer linii bufora z wolną zawartością).
