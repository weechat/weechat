...
[NOTE]
^(1)^ Der Name stammt von der Debian GNU/Linux Trixie-Distribution,
...
include::includes/key_bindings.de.adoc[]
...
Ein Theme ist eine Zusammenstellung von Optionen die angepasst werden und
die mithilfe des Befehls <<command_weechat_theme,/theme>> angewendet werden
kann. WeeChat beinhaltet ein integriertes Theme namens `"light"`, das für
Terminals mit hellem Hintergrund optimiert wurde und unterstützt
benutzerdefinierte Themes, die temporär aus Dateien geladen werden können.

[[themes_themable_options]]
==== Anpassbare Optionen

Themes können nur Optionen speichern und verändern, die als _themable_ (themenfähig)
gekennzeichnet sind. Alle `*.color.*`-Optionen sind standardmäßig
themenfähig; einige wenige String-Optionen, die Formatausdrücke mit
`+${color:...}+`-Referenzen enthalten (wie etwa `+weechat.color.chat_nick_colors+`, `+weechat.look.prefix_error+`
oder die `+buflist.format.*+`-Formate), wurden hierfür explizit freigeschaltet.

Sie können alle Optionen, die ein Theme anpassen kann, im
<<command_fset_fset,/fset>> Buffer mit dem `+t:themable+` Filter anzeigen lassen.

[[themes_apply]]
==== Applying a theme

So wechseln Sie zum integrierten hellen Hintergrunddesign:

----
/theme apply light
----

Der aktuelle Zustand aller anpassbaren Optionen wird vorab in einer
Backup-Theme-Datei mit dem Namen `+backup-YYYYMMDD-HHMMSS-uuuuuu+`
im Verzeichnis `+themes+` innerhalb des WeeChat-Konfigurations-
verzeichnisses gespeichert, sodass das vorherige Erscheinungsbild
jederzeit wie folgt wiederhergestellt werden kann:

----
/theme apply backup-YYYYMMDD-HHMMSS-uuuuuu
----

Die Erstellung von Backups kann deaktiviert werden (nicht empfohlen):

----
/set weechat.look.theme_backup off
----

Wenn die Datensicherung aktiviert ist und die Sicherungsdatei nicht
geschrieben werden kann, wird der Vorgang abgebrochen, bevor eine
Option geändert wird.

Der Name des zuletzt angewendeten Themes wird in `+weechat.look.theme+`
gespeichert (nur zur Information; wird beim Start nicht erneut angewendet).

[[themes_reset]]
==== Auf Standardeinstellungen zurücksetzen

Um das mit WeeChat ausgelieferte Erscheinungsbild wiederherzustellen,
setzen Sie alle anpassbaren Optionen auf ihren Standardwert zurück:

----
/theme reset
----

Zunächst wird ein Backup erstellt (gleicher Schritt wie bei `+/theme apply+`);
schlägt das Backup fehl, wird der Vorgang abgebrochen, bevor eine
Option geändert wird. Auch `+weechat.look.theme+` wird zurückgesetzt.

[[themes_save_delete]]
==== Speichern und Löschen von Benutzer-Themes

Speichern Sie die aktuellen anpassbaren Optionen als neue Benutzer-Theme-Datei:

----
/theme save mytheme
----

Alle anpassbaren Optionen sind definiert, sodass die Datei in sich geschlossen
ist und in jeder WeeChat-Instanz exakt dasselbe Erscheinungsbild erzeugt –
unabhängig von der jeweiligen Konfiguration.

Reservierte Namen (integrierte Theme-Namen wie `+light+` sowie alle Namen, die
mit `+backup-+` beginnen) werden abgelehnt. Die Dateien befinden sich unter
`+${weechat_config_dir}/themes/<name>.theme+`.

Benennen Sie ein Benutzer-Theme um (typischer Anwendungsfall: Aufbewahrung
eines nützlichen automatisch erstellten Backups unter einem aussagekräftigen Namen):

----
/theme rename backup-20260525-094210-123456 mybackup
----

Integrierte Themes besitzen keine Datei und können nicht umbenannt werden;
der Zielname darf nicht mit einem integrierten Namen übereinstimmen oder
mit `+backup-+` beginnen, und die Zieldatei darf noch nicht existieren.
Das Feld `+name+` im Abschnitt `+[info]+` der Datei wird aktualisiert,
sodass `/theme info` den neuen Namen konsistent anzeigt.

Benutzer-Theme löschen:

----
/theme del mytheme
----

Dadurch wird die Datei auf dem Datenträger entfernt; integrierte Themes
können nicht gelöscht werden.

[[themes_file_format]]
==== Theme Dateiformat

Eine Themendatei ist INI-artig aufgebaut und besteht aus zwei Abschnitten:
...
`+[info]+` sind informative Metadaten, die durch `/theme info` angezeigt werden. `+[options]+`
enthält die eigentlichen Anpassungen; bei den Schlüsseln handelt es sich um die vollständigen
Optionsnamen, wie sie auch bei `/set` verwendet werden. Zeichenkettenwerte können in doppelte
oder einfache Anführungszeichen gesetzt werden; diese werden beim Parsen entfernt. Optionen,
die nicht über ein Theme angepasst werden können, aber dennoch in einer Theme-Datei aufgeführt
sind, werden beim Anwenden abgelehnt und im Core-Buffer protokolliert (sodass eine aus einer
nicht vertrauenswürdigen Quelle importierte `.theme`-Datei keine Passwörter, Autoload-Listen
oder Startbefehle überschreiben kann).

Benutzer-Theme-Dateien werden niemals zwischengespeichert: Bei jedem
`/theme apply <name>` wird die Datei geparst, angewendet und wieder
freigegeben.

[[themes_resolution]]
==== Resolution order

Wenn `+/theme apply <name>+` ausgeführt wird:


* Falls `+${weechat_config_dir}/themes/<name>.theme+` existiert, wird die Datei
  geparst und verwendet (sie überlagert ein etwaiges eingebautes Theme gleichen Namens).
* Andernfalls wird die interne Theme-Registratur herangezogen; interne Themes
  werden programmatisch durch Core, Erweiterungen und Skripte registriert (siehe
  die Erweitung- und Skripting-Dokumentation zu `+weechat_theme_register+`).

Liefert keine der beiden Quellen den Namen, wird die Ausführung des Befehls abgelehnt.
...
`man strftime` für das Format, extra specifiers are supported, see link:weechat_plugin_api.en.html#_util_strftimeval[WeeChat plugin API reference / util_strftimeval ^↗^^]). Zum Beispiel:
...
| id                 | string  | Zeilenkennung: Datum und Uhrzeit des Ausdrucks mit Mikrosekunden-Genauigkeit (Zeilennummer bei einem Buffer mit freiem Inhalt).
