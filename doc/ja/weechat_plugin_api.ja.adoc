...
| `+${xxx}+` |
| バッファに対するローカル変数 `xxx` の値
| >> `+${nick}+` +
  == `+FlashCode+`

| `+${xxx}+` |
| _pointers_ の変数 `xxx` の値に展開
| >> `+${buffer}+` +
  == `+0x1234abcd+`
...
| _chat_line_date_printed | 日付/時間の行 ^(5)^
| "1313237175" | "0"
...
* _infolist_: インフォリストへのポインタ
// TRANSLATION MISSING
* _var_: variable name (must be type "long")
...
* _infolist_: インフォリストへのポインタ
// TRANSLATION MISSING
* _var_: variable name (must be type "long long")
