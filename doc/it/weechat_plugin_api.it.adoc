...
_WeeChat ≥ 2.4, updated in 4.3.0, 4.11.0._
...
// TRANSLATION MISSING
If the string to decode is invalid (invalid char, truncated string or invalid
padding), an empty string is stored in _*to_ and -1 is returned.

// TRANSLATION MISSING
Missing padding chars (`=`) at the end of string are accepted.
...
Special variables with format `+${xxx}+` are expanded (see table below).
...
| `+${xxx}+` |
| Variable `xxx` from hashtable _extra_vars_.
| >> `+${xxx}+` +
  == `+value+`
...
| `+${xxx}+` |
| Value of local variable `xxx` in buffer.
| >> `+${nick}+` +
  == `+FlashCode+`

| `+${xxx}+` |
| Variable `xxx` from hashtable _pointers_.
| >> `+${buffer}+` +
  == `+0x1234abcd+`
...
| Hdata value (pointers `window` and `buffer` are set by default with current
  ...
  For name of hdata and variables, see function <<_hdata_get,hdata_get>>.
...
// TRANSLATION MISSING
Call a function on all hashtable entries, sorted by key (with the compare
function on keys given when the hashtable was created).
...
entry that has not been sent to it yet (removing the entry received or an entry
already received is safe).
...
| id
| Line identifier: date and time of print with microseconds precision.
| Line number ("y").
| `+1786886000000000+`
...
// TRANSLATION MISSING
| _chat_line_id | Line identifier ^(4)^ _(WeeChat ≥ 4.11.0)_.
| "1786886000000000" | "-1"
...
| _chat_line_date_printed | Riga con data/ora ^(5)^.
| "1313237175" | "0"

| _chat_line_date_usec_printed | Microseconds of line printed date/time ^(5)^.
| "123456" | "0"
...
^(4)^ On buffers with formatted content, this is the date when WeeChat added
the line in the buffer, with microseconds precision (greater or equal to
"_chat_line_date"); on buffers with free content, this is the line number
("_chat_line_y"). +
^(5)^ Deprecated since WeeChat 4.11.0 (still there for compatibility): it is
derived from the line identifier ("_chat_line_id") and is "0" on buffers with
free content.
...
// TRANSLATION MISSING
** _prefix_for_each_line_: 1 if prefix is displayed for each line in buffer (default), otherwise 0 _(WeeChat ≥ 4.11.0)_
...
// TRANSLATION MISSING
Return value of a "long long" buffer property.
...
// TRANSLATION MISSING
** _lines_last_id_assigned_: last unique identifier used in this buffer for a line _(WeeChat ≥ 4.11.0)_
** _nicklist_last_id_assigned_: last unique identifier used in this buffer for a group or nick

Valore restituito:

// TRANSLATION MISSING
* "long long" value of property

Esempio in C:

[source,c]
----
weechat_printf (NULL, "my buffer id is: %lld",
                weechat_buffer_get_longlong (my_buffer, "id"));
----
...
// TRANSLATION MISSING
| prefix_for_each_line | 4.11.0 | "0" or "1"
| "0" to hide prefix for all lines in buffer, "1" to see prefix for all lines
  (default for a new buffer).
...
_WeeChat ≥ 4.4.0, updated in 4.11.0._
...
* _buffer_: puntatore al buffer
// TRANSLATION MISSING
* _id_: line identifier: on buffer with formatted content, this is the date
  and time of print with microseconds precision (number of microseconds
  since the Epoch); on buffer with free content, this is the line number ("y")
...
// TRANSLATION MISSING
Add a "long" variable to an infolist item.
...
// TRANSLATION MISSING
Add a "long long" variable to an infolist item.
...
// TRANSLATION MISSING
Return value of "long" variable in current infolist item.
...
* _infolist_: puntatore alla lista info
* _var_: nome della variabile (deve essere di tipo "long")
...
weechat_printf (NULL, "value = %ld",
                weechat_infolist_long (infolist, "my_long"));
----
...
Return value of "long long" variable in current infolist item.
...
* _infolist_: puntatore alla lista info
* _var_: nome della variabile (deve essere di tipo "long long")
...
weechat_printf (NULL, "value = %lld",
                weechat_infolist_longlong (infolist, "my_longlong"));
----
