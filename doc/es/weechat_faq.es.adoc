...
Si el servidor soporta SASL, debería usar esto en vez de enviar el
...
Si el servidor no soporta SASL, puede añadir un retraso (entre el comando y
...
El comando `/ignore` es un comando de IRC, así que solo se aplica en los búfers de IRC
...
El comando `/filter` es un comando del núcleo de WeeChat, así que se aplica a cualquier búfer.
Le permite filtar algunas líneas de los búfers con etiquetas o expresiones regulares para
...
Para ocultar mensajes (pero mantenerlos en el búfer), puede filtrarlos utilizando la etiqueta propia del mensaje
