...
[verse]
*weechat-headless* [-a|--no-connect] [--daemon] [--stdout] [-d|--dir <путь>] [-t|--temp-dir] [-p|--no-plugin] [-P|--plugins <плагины>] [-r|--run-command <команда>] [-s|--no-script] [--upgrade] [параметр-отладки...] [плагин:параметр...]
...
*ВНИМАНИЕ:* эти параметры предназначены только для отладки, *НЕ ИСПОЛЬЗУЙТЕ В РЕЖИМЕ ЭКСПЛУАТАЦИИ!*
