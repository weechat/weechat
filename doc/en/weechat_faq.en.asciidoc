...
[[otr_encryption]]
=== How can I encrypt my messages using OTR (Off-The-Record) messaging? ===

WeeChat doesn't support OTR directly yet
(link:https://github.com/weechat/weechat/issues/298[issue 298]), but there
is a script (otr.py) for it.

First you need to install the requirements for the script. As
pre-requirements, you should have (Debian/Ubuntu) packages
`build-essential`, `python-dev` and `python-pip` installed and then you can
install the real requirements with `pip install -r https://github.com/mmb/weechat-otr/raw/master/requirements.txt --user --upgrade`.
