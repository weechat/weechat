...
It's useful for modifiers and providers, because execution order is important.
...
config, completion, modifier, provider, info, info_hashtable, infolist, hdata,
focus.
...
[[signal_irc_nick_refresh_color]]
===== Signal irc_nick_refresh_color

_WeeChat ≥ x.y.z._

The signal "irc_nick_refresh_color" can be sent to force refresh of cached
IRC nick colors.

Following argument formats are allowed:
...
[source,C]
----
/* force nick color refresh on freenode server, #weechat channel */
weechat_hook_signal_send ("irc_nick_refresh_color", WEECHAT_HOOK_SIGNAL_STRING,
                          "freenode;#weechat");
----
...
----
# force nick color refresh on freenode server, #weechat channel
weechat.hook_signal_send ("irc_nick_refresh_color", weechat.WEECHAT_HOOK_SIGNAL_STRING,
                          "freenode;#weechat")
----
...
[source,C]
----
struct t_hook *weechat_hook_provider (const char *provider,
...
* 'provider': provider name, list of providers used by Weechat or plugins
  (priority allowed, see note about <<hook_priority,priority>>)
  (see table below)
* 'callback': function called when provider is used, arguments and return value:
** 'void *data': pointer
** 'const char *provider': name of provider
** 'const char *provider_data': data for provider
** 'const char *string': string to provide for
** return value: provided string, or empty string when not providing
* 'callback_data': pointer given to callback when it is called by WeeChat
...
List of providers used by WeeChat and plugins:

[width="100%",cols="^2,3,4,4",options="header"]
|===
| provider | provider data | String | Output

| irc_nick_color |
  |
  Nickname to provide color for |
  Color name provided for nickname
|===
...
----
const char *
my_provider_cb (void *data, const char *provider,
                const char *provider_data,
                const char *string)
{
    static const char *my_color = "red";

    return my_color;
}

struct t_hook *my_provider_hook = weechat_hook_provider ("9000|irc_nick_color",
                                                         &my_provider_cb, NULL);
----
...
----
# prototype
hook = weechat.hook_provider(provider, callback, callback_data)

# example
def my_provider_cb(data, provider, provider_data, string):
    return "red"

hook = weechat.hook_provider("9000|irc_nick_color", "my_provider_cb", "")
----
...
[source,C]
----
char *weechat_hook_provider_exec (const char *provider,
                                  const char *provider_data,
                                  const char *string);
----
...
* string provided, NULL if error occurred

C example:

[source,C]
----
char *new_string = weechat_hook_provider_exec ("my_provider",
                                               my_data, my_string);
----
...
----
# prototype
weechat.hook_provider_exec(provider, provider_data, string)

# example
weechat.hook_provider_exec("my_provider", my_data, my_string)
----
