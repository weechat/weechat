...
[NOTE]
^(1)^ Name comes from the Debian GNU/Linux Trixie distribution, version and
...
include::includes/key_bindings.en.adoc[]
...
`man strftime` for formats, extra specifiers are supported, see link:weechat_plugin_api.en.html#_util_strftimeval[WeeChat plugin API reference / util_strftimeval ^↗^^]),
for example:
...
| id                 | string  | Line identifier: date and time of print with microseconds precision (line number for a buffer with free content).
