...
/* Remplacer les codes couleur par "?". */
...
_WeeChat ≥ 2.4, mis à jour dans la 4.3.0, 4.11.0._
...
Si la chaîne à décoder est invalide (caractère invalide, chaîne tronquée ou
remplissage invalide), une chaîne vide est stockée dans _*to_ et -1 est
retourné.

Les caractères de remplissage (`=`) manquants à la fin de la chaîne sont
acceptés.
...
Les variables spéciales avec le format `+${xxx}+` sont étendues (voir le
tableau ci-dessous).
...
struct t_hashtable *options2 = weechat_hashtable_new (8,
                                                      ...
                                                      NULL,
                                                      NULL);
/* Ajouter des crochets autour des URLs. */
...
# Remplacer avec regex : ajout de crochets autour des URLs.
options = {
    "regex": "[a-zA-Z0-9_]+://[^ ]+",
    "regex_replace": "[ ${re:0} ]",
}
...
# Remplacer avec regex : masquer des mots de passe.
options = {
    "regex": "(password=)([^ ]+)",
...
| `+${xxx}+` |
| Variable `xxx` de la table de hachage _extra_vars_.
| >> `+${xxx}+` +
  == `+valeur+`
...
| `+${sec.data.xxx}+` |
| Valeur de la donnée sécurisée `xxx`.
...
| `+${xxx}+` |
| Valeur de la variable locale `xxx` dans le tampon.
| >> `+${nick}+` +
  == `+FlashCode+`

| `+${xxx}+` |
| Variable `xxx` de la table de hachage _pointers_.
| >> `+${buffer}+` +
  == `+0x1234abcd+`
...
| Valeur d'un hdata (les pointeurs `window` et `buffer` sont définis par défaut
  ...
  Pour le nom du hdata et des variables, voir la fonction <<_hdata_get,hdata_get>>.
...
* _filename1_ : premier fichier à comparer
* _filename2_ : second fichier à comparer

Valeur de retour :

* 0 : les deux fichiers ont le même contenu
* 1 : le contenu est différent
* 2 : erreur (fichier non trouvé ou erreur de lecture)
...
** `%.N` où `N` est entre 1 et 6 : microsecondes remplies avec des zéros sur
...
[source,c]
----
int index = weechat_arraylist_insert (arraylist, -1, pointer);  /* Insérer à la fin si non trié. */
----
...
Appeller une fonction pour chaque entrée d'une table de hachage, triées par
clé (avec la fonction de comparaison sur les clés donnée à la création de la
table de hachage).
...
[source,c]
----
void weechat_hashtable_map_sorted (struct t_hashtable *hashtable,
                                   void (*callback_map)(void *data,
                                                        struct t_hashtable *hashtable,
                                                        const void *key,
                                                        const void *value),
...
[NOTE]
La liste des entrées est construite avant le premier appel à la fonction de
rappel : les entrées ajoutées par la fonction de rappel ne lui sont pas
envoyées, et la fonction de rappel ne doit pas supprimer une entrée qui ne lui
a pas encore été envoyée (supprimer l'entrée reçue ou une entrée déjà reçue ne
...
/* ... */
weechat_hashtable_map_sorted (hashtable, &map_cb, NULL);
----
...
Retourner la valeur entière héritée de l'option : valeur de l'option si elle
n'est pas NULL, ou valeur de l'option parente (si l'option hérite d'une autre
option). +
Si l'option parente n'est pas trouvée, retourner la valeur par défaut de
l'option. +
Si la valeur parente est NULL, retourner la valeur par défaut de l'option
parente.
...
Retourner la valeur chaîne héritée de l'option : valeur de l'option si elle
n'est pas NULL, ou valeur de l'option parente (si l'option hérite d'une autre
option). +
Si l'option parente n'est pas trouvée, retourner la valeur par défaut de
l'option. +
...
Retourner la valeur couleur héritée de l'option : valeur de l'option si elle
n'est pas NULL, ou valeur de l'option parente (si l'option hérite d'une autre
option). +
Si l'option parente n'est pas trouvée, retourner la valeur par défaut de
l'option. +
...
Retourner la valeur énumérée héritée de l'option : valeur de l'option si elle
n'est pas NULL, ou valeur de l'option parente (si l'option hérite d'une autre
option). +
Si l'option parente n'est pas trouvée, retourner la valeur par défaut de
l'option. +
...
| id
| Identifiant de la ligne : date et heure d'affichage avec une précision à la microseconde.
| Numéro de la ligne (« y »).
| `+1786886000000000+`
...
| weechat | [[hook_signal_buffer_prefix_for_each_line_changed]] buffer_prefix_for_each_line_changed | 4.11.0
| Pointeur : tampon.
| Affichage du préfixe pour chaque ligne changé.
...
[source,c]
----
/* Décharger et supprimer les scripts test.py et script.py. */
...
[source,c]
----
/* Dire "Bonjour !" sur le serveur libera, canal #weechat. */
weechat_hook_signal_send ("irc_input_send", WEECHAT_HOOK_SIGNAL_STRING,
                          "libera;#weechat;priority_high,user_message;;Bonjour !");

/* Envoyer la commande "/whois FlashCode" sur le canal libera, en basse priorité. */
weechat_hook_signal_send ("irc_input_send", WEECHAT_HOOK_SIGNAL_STRING,
...
if (hashtable)
{
    ...
    /*
     * Reediriger maintenant la commande irc whois avec le hsignal irc_redirect_command,
...
/* Ajouter l'information "mon_info". */
...
/* Ajouter l'information "mon_info_hashtable". */
...
/* Ajouter l'infolist "mon_infolist". */
...
| _chat_line_id | Identifiant de la ligne ^(4)^ _(WeeChat ≥ 4.11.0)_.
| "1786886000000000" | "-1"
...
| _chat_line_date_printed | Date/heure de la ligne ^(5)^.
| "1313237175" | "0"

| _chat_line_date_usec_printed | Microsecondes de la date/heure de la ligne ^(5)^.
| "123456" | "0"
...
^(4)^ Sur un tampon avec contenu formaté, il s'agit de la date à laquelle
WeeChat a ajouté la ligne dans le tampon, avec une précision à la microseconde
(supérieure ou égale à "_chat_line_date") ; sur un tampon avec contenu libre,
il s'agit du numéro de la ligne ("_chat_line_y"). +
^(5)^ Obsolète depuis WeeChat 4.11.0 (toujours présent pour la compatibilité) :
il est déduit de l'identifiant de la ligne ("_chat_line_id") et vaut "0" sur
les tampons avec contenu libre.
...
/* Ajouter le focus sur la liste des pseudos. */
...
# Exemple: mélanger le tampon courant avec le tampon "core"
...
** _prefix_for_each_line_ : 1 si le préfixe est affiché pour chaque ligne du tampon (par défaut), sinon 0 _(WeeChat ≥ 4.11.0)_
...
# Exemple
weechat.prnt("", "mon numéro de tampon est : %d" % weechat.buffer_get_integer(mon_tampon, "number"))
----
...
Retourner une valeur entière de type "long long" pour une propriété du tampon.

Prototype :

[source,c]
----
long long weechat_buffer_get_longlong (struct t_gui_buffer *buffer,
                                       const char *property);
----
...
* _buffer_ : pointeur vers le tampon
* _property_ : nom de la propriété :
** _id_ : identifiant unique
** _lines_last_id_assigned_ : dernier identifiant unique utilisé dans ce tampon pour une ligne _(WeeChat ≥ 4.11.0)_
** _nicklist_last_id_assigned_ : dernier identifiant unique utilisé dans ce tampon pour un groupe ou un pseudo

Valeur de retour :

* valeur entière de la propriété sous forme de "long long"

Exemple en C :

[source,c]
----
weechat_printf (NULL, "mon id de tampon est : %lld",
                weechat_buffer_get_longlong (mon_tampon, "id"));
----
...
# Exemple
weechat.prnt("", "mon id de tampon est : %d" % weechat.buffer_get_longlong(mon_tampon, "id"))
----
...
weechat.prnt("", "nom / nom court du tampon sont : %s / %s"
    % (weechat.buffer_get_string(mon_tampon, "name"),
    weechat.buffer_get_string(mon_tampon, "short_name")))
----
...
| prefix_for_each_line | 4.11.0 | "0" ou "1"
| "0" pour cacher le préfixe sur toutes les lignes du tampon, "1" pour afficher
  le préfixe sur toutes les lignes (par défaut pour un nouveau tampon).
...
_WeeChat ≥ 4.4.0, mis à jour dans la 4.11.0._
...
[source,c]
----
struct t_gui_line *weechat_line_search_by_id (struct t_gui_buffer *buffer,
                                              long long id);
----
...
* _buffer_ : pointeur vers le tampon
* _id_ : identifiant de la ligne : sur un tampon avec contenu formaté,
  c'est la date et l'heure d'affichage avec une précision à la microseconde
  (nombre de microsecondes depuis l'Epoch) ; sur un tampon avec contenu
  libre, c'est le numéro de la ligne (« y »)
...
[source,c]
----
struct t_gui_line *line = weechat_line_search_by_id (buffer, 1786886000000000);
----
...
# Exemple
def my_completion_cb(data: str, completion_item: str, buffer: str, completion: str) -> int:
    # Récupèrer les paramètres de la commande.
...
| add_space | 4.6.0 | "0" ou "1"
| "0" : ne pas ajouter d'espace après la complétion +
  "1" : ajouter un espace après la complétion (par défaut).
|===
...
==== infolist_new_var_long

_WeeChat ≥ 4.11.0._

Ajouter une variable de type "long" dans l'objet de l'infolist.

Prototype :

[source,c]
----
struct t_infolist_var *weechat_infolist_new_var_long (struct t_infolist_item *item,
                                                      const char *name,
                                                      long value);
----
...
[source,c]
----
struct t_infolist_var *var = weechat_infolist_new_var_long (item,
                                                            "mon_long",
                                                            2123456789);
----
...
# Exemple
var = weechat.infolist_new_var_long(item, "mon_long", 2123456789)
----

==== infolist_new_var_longlong

_WeeChat ≥ 4.11.0._

Ajouter une variable de type "long lon" dans l'objet de l'infolist.

Prototype :

[source,c]
----
struct t_infolist_var *weechat_infolist_new_var_longlong (struct t_infolist_item *item,
                                                          const char *name,
                                                          long long value);
----
...
* _item_ : pointeur vers l'objet de l'infolist
* _name_ : nom de la variable
* _value_ : valeur de type "long long"
...
[source,c]
----
struct t_infolist_var *var = weechat_infolist_new_var_longlong (item,
                                                                "mon_longlong",
                                                                9123456789123456789);
----
...
# Exemple
var = weechat.infolist_new_var_longlong(item, "mon_longlong", 9123456789123456789)
----
...
[source,c]
----
long weechat_infolist_long (struct t_infolist *infolist, const char *var);
----

Paramètres :

* _infolist_ : pointeur vers l'infolist
* _var_ : nom de la variable (doit être de type "long")

Valeur de retour :

* valeur de la variable sous forme de "long"

Exemple en C :

[source,c]
----
weechat_printf (NULL, "value = %ld",
                weechat_infolist_long (infolist, "mon_long"));
----
...
# Exemple
weechat.prnt("", "value = %d" % weechat.infolist_long(infolist, "mon_long"))
----
...
[source,c]
----
long long weechat_infolist_longlong (struct t_infolist *infolist, const char *var);
----

Paramètres :

* _infolist_ : pointeur vers l'infolist
* _var_ : nom de la variable (doit être de type "long long")

Valeur de retour :

* valeur de la variable sour forme de "long long"

Exemple en C :

[source,c]
----
weechat_printf (NULL, "value = %lld",
                weechat_infolist_longlong (infolist, "mon_longlong"));
----
...
# Exemple
weechat.prnt("", "value = %d" % weechat.infolist_longlong(infolist, "mon_longlong"))
----
...
[source,c]
----
/* Vérifier si le pointeur vers le tampon est valide. */
struct t_hdata *hdata = weechat_hdata_get ("buffer");
...
/* Chercher un serveur irc avec le nom "libera". */
...
[source,c]
----
/* Soustraire une heure sur le dernier message affiché dans le tampon courant. */
