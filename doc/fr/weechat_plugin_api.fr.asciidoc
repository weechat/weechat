...
[source,C]
----
weechat_printf (NULL, "user_data valeur de l'item: %p", weechat_list_user_data (item));
----
...
** 'priority_high' : file d'attente avec haute priorité (comme les messages
   utilisateur) ; c'est la priorité par défaut
** 'priority_low' : file d'attente avec basse priorité (comme les messages
...
*** 'WEECHAT_RC_OK'
*** 'WEECHAT_RC_ERROR'
* 'input_callback_pointer' : pointeur donné au "callback" lorsqu'il est appelé
  par WeeChat
...
*** 'WEECHAT_RC_OK'
*** 'WEECHAT_RC_ERROR'
* 'close_callback_pointer' : pointeur donné au "callback" lorsqu'il est appelé
  par WeeChat
...
** 'short_name' : nom court du tampon (note : utilisé pour l'affichage
   seulement et peut être changé par l'utilisateur, il ne doit pas être utilisé
   pour trouver le nom du tampon, utilisez à la place 'name', 'full_name' ou
   bien la variable locale 'channel')
...
* 'build_callback_pointer' : pointeur donné au "callback" lorsqu'il est appelé
  par WeeChat
