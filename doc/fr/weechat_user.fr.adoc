...
[NOTE]
^(1)^ Le nom provient de la distribution GNU/Linux Debian Trixie, la version
...
include::includes/key_bindings.fr.adoc[]
...
dans le masque (voir `man strftime` pour le format, des marqueurs
supplémentaires sont supportés, voir la link:weechat_plugin_api.fr.html#_util_strftimeval[Référence API extension WeeChat / util_strftimeval ^↗^^]),
par exemple :
...
| Chaîne : pointeur tampon + ";" + état (un parmi ceux-ci : "off", "typing",
  ...
  Exemple : "0x1234abcd;typing;alice".
...
| line           | message             | buffer, buffer_name, id, y, date, date_usec, str_time, tags, notify_level, highlight, prefix, message
| print          | tg_message          |
...
| id                 | chaîne   | Identifiant de la ligne : date et heure d'affichage avec une précision à la microseconde (numéro de la ligne pour un tampon avec contenu libre).
