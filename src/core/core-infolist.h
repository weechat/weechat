 ...
 * "value"/"pointer" either); the infolist takes ownership of the pointer(s) and
 * the caller must never use or free them afterwards: on success they are freed
 * later by infolist_var_free(), and on error (NULL return) they are freed
 * immediately by the function itself.
