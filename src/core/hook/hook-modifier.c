...
struct t_hdata *
hook_modifier_hdata_hook_modifier_cb (const void *pointer, void *data,
                                      const char *hdata_name)
{
    ...
    if (hdata)
    {
        HDATA_VAR(struct t_hook_modifier, callback, POINTER, 0, NULL, NULL);
        HDATA_VAR(struct t_hook_modifier, modifier, STRING, 0, NULL, NULL);
    }
    return hdata;
}
