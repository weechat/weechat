...
{
    const char *ptr_command, *ptr_filename;
    const char *ptr_concat_separator, *ptr_strip_chars;
    char *command, space[2] = " ";
    ...
    if (!gui_chat_pipe)
    {
        ...
        else
        {
            ...
            {
                gui_chat_printf (NULL,
                                 _("%sPipe can only be done on buffers with "
                                   "formatted content"),
                                 gui_chat_prefix[GUI_CHAT_PREFIX_ERROR]);
...
command_init ()
{
    ...
    hook_command (
        NULL, "pipe",
        N_("redirect command output to a buffer or a file"),
        /* TRANSLATORS: only text between angle brackets (eg: "<name>") may be translated */
        N_("[-buffer <name>|-file <filename>] [-concat <separator>] "
        ...
        CMD_ARGS_DESC(
            ...
            N_("separator: separator for concatenated lines; chars can be escaped "
               "(example: \\x20 for space)"),
            N_("raw[-strip]: strip chars from lines (left/right)"),
            N_("chars: chars to strip; chars can be escaped "
               "(example: \\x20 for space)"),
            ...
            N_("command: command to execute (a \"/\" is automatically added "
               "if not found at beginning of command)"),
            "",
            N_("If no target is specified (\"-buffer\" or \"-file\"), then the "
               "output is sent on the current buffer."),
            "",
            N_("Note: commands that display messages in an asynchronous way (like "
               "/exec and many IRC commands), the output will not be caught by "
               "this command."),
