...
def check_req_symbols(symbols: List[WeechatCurlSymbol]) -> int:
    ...
    for symbol in symbols:
        ...
        if symbol.max_curl and symbol.max_curl <= req_curl:
            print(
                f"{SRC_PATH}:{symbol.line_no}: "
                f"symbol {symbol.name} max version {to_str(symbol.max_curl)} is "
                f"older/equal to the minimal required curl {to_str(req_curl)}. "
        ...
        if symbol.min_curl != curl_symbol[0]:
            if not symbol.min_curl:
                if req_curl >= curl_symbol[0]:
                    print(
                        f"{SRC_PATH}:{symbol.line_no}: "
                        f"symbol {symbol.name} is missing appropriate min "
                        f"version if guard {curl_symbol[0]}"
                    )
                    errors += 1
            else:
                print(
        ...
        if symbol.max_curl != curl_symbol[1]:
            if not symbol.max_curl:
                if req_curl >= curl_symbol[1]:
                    print(
                        f"{SRC_PATH}:{symbol.line_no}: "
                        f"symbol {symbol.name} is missing appropriate max "
                        f"version if guard {curl_symbol[1]}"
                    )
                    errors += 1
            else:
                print(
